...
{
	uint8_t* src = (uint8_t *)gbk;
	uint8_t buf[2];
	uint16_t temp;

	while (*src != 0)
	{
		if (*src < 0X81)				// ascii code
		{
			temp = *src++;
		} else							// gbk code
		{
...
void unistr2gbkstr(void* dst, const void* src)
{
	...
	while (*s != 0)
	{
		...
		buf[0] = chr2hex(*s++) << 4;
		buf[0] += chr2hex(*s++);
		temp = ff_convert(*(uint16_t *)buf, 0);
...
void gbkstr2unistr(void* dst, const void* src)
{
	...
	while (*s != 0)
	{
		if (*s < 0X81)									// ascii
		{
			temp = ff_convert((uint16_t) * s, 1);
			s++;
		} else											// gbk
		{
			buf[1] = *s++;
			buf[0] = *s++;
			temp = ff_convert(*(uint16_t*)buf, 1);
		}
		*d++ = hex2chr((temp >> 12) & 0X0F);
		*d++ = hex2chr((temp >> 8) & 0X0F);
 ...
 * UNICODE和OEMCP互转函数
 * @param  src 待转换的值
 * @param  dir 0：unicode转oemcp  1：oemcp转unicode
 * @return     转换后的值
 */
